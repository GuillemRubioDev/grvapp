...
const ToastGrv = ({ message, type = "success", duration = 3000, onClose }: ToastGrvProps) => {
    ...
    const toastGrv = (
        <div
            className={`fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white dark:bg-dark-surface text-text dark:text-dark-text border-l-4 ${type === "success"
                ...
                } px-5 py-3 rounded-xl shadow-lg flex flex-row items-center gap-3 z-[9999] min-w-[280px] max-w-[90vw] ${fadeOut ? "animate-fade-out" : "animate-fade-in"
                }`}
        >
    ...
    return toastRoot ? ReactDOM.createPortal(toastGrv, toastRoot) : null;
};

export default ToastGrv;